...
{
    public class PatcherModder : MonoModder
    {
        ...
        public PatcherModder() : base()
        {
            PostProcessors += FMODPostProcessor;
        }
        ...
        {
			if (method.Name.Contains("create", StringComparison.InvariantCultureIgnoreCase)) {
				Console.WriteLine($"guhhh {method.GetID(simple: true)}");
			}
        ...
        public override void MapDependencies()
        {
            ...
            _CoreifierModule ??= ModuleDefinition.ReadModule("/bin/CelestePatcher.dll") ?? throw new Exception("Failed to load Celeste Patcher assembly");
        ...
        public override void AutoPatch()
        {
            ...
            ParseRules(DependencyMap[Module].First(dep => dep.Assembly.Name.Name == "CelestePatcher"));
