...
namespace NETCoreifier {
    public class NetFrameworkModder : MonoModder {
        ...
        public override void MapDependencies() {
            ...
            AddReferenceIfMissing(Assembly.GetEntryAssembly().GetName());

            // We have to load our own module again every time because MonoMod messes with it ._.
            _CoreifierModule ??= ModuleDefinition.ReadModule("/bin/NETCoreifier.dll") ?? throw new Exception("Failed to load .NET Coreifier assembly");
            DependencyCache[Assembly.GetEntryAssembly().FullName] = _CoreifierModule;
